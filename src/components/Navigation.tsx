      ...
      <div className="container mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-8 relative">
            {navItems.map((item, index) => <motion.a key={item.label} href={item.href} onClick={e => {
            e.preventDefault();
            scrollToSection(item.href);
          }} className="text-sm font-medium text-foreground hover:text-primary transition-smooth relative group" initial={{
            opacity: 0,
            y: -20
          }} animate={{
            opacity: 1,
            ...
            y: -2
                ...
                {item.label}
                <motion.span className="absolute bottom-0 left-0 h-0.5 bg-gradient-to-r from-primary to-accent" initial={{
              width: 0
            }} animate={{
        ...
        {/* Mobile Navigation */}
        {isMobileMenuOpen && <motion.div className="md:hidden mt-4 py-4 space-y-4 border-t border-border" initial={{
        opacity: 0,
        height: 0
      }} animate={{
        opacity: 1,
        ...
        height: 0
          ...
          e.preventDefault();
          scrollToSection(item.href);
        }} className="block text-sm font-medium text-foreground hover:text-primary transition-smooth" initial={{
          opacity: 0,
          x: -20
        }} animate={{
          opacity: 1,
